...
The checked source is [crates/rustium-cli/examples/embed_postgresql.rs](crates/rustium-cli/examples/embed_postgresql.rs). Place the connector configuration at `rustium.yaml` and run it with `cargo run -p rustium-cdc --example embed_postgresql`. The published crate is `rustium-cdc`; it installs the `rustium` binary.
...
Install the published command-line package from crates.io:

```bash
cargo install rustium-cdc --locked
rustium validate --config examples/postgresql.yaml
```

```bash
# Validate configuration and external dependencies.
cargo run -p rustium-cdc -- validate --config examples/postgresql.yaml

# Run one connector in the foreground.
cargo run -p rustium-cdc -- run --config examples/postgresql.yaml

# Explicitly remove one connector checkpoint.
cargo run -p rustium-cdc -- state reset \
...
PostgreSQL, MySQL, and SQL Server column transformations use Debezium's dynamic property names. Selectors are anchored, case-insensitive regular expressions over `schema.table.column` for PostgreSQL, `database.table.column` for MySQL, and either `database.schema.table.column` or `schema.table.column` for SQL Server; the first matching rule in the fixed category order `truncate -> fixed mask -> hash V1 -> hash V2` wins. In Java `.properties` files, write `\\.` when a selector must retain the regular-expression escape for a literal dot. `column.truncate.to.<length>.chars` truncates character or binary values, `column.mask.with.<length>.chars` emits that many `*` characters even for SQL NULL, and both hash forms preserve NULL. Hashes accept the common JCA digest names (`MD2`, `MD5`, `SHA-1`, `SHA-224`, `SHA-256`, `SHA-384`, `SHA-512`, `SHA-512/224`, `SHA-512/256`, and SHA-3 variants); V1 uses Java `ObjectOutputStream` String serialization compatibility bytes, while V2 hashes UTF-8 text. Hash output is lowercase hexadecimal and is shortened to a declared `char(n)`, `varchar(n)`, `nchar(n)`, or `nvarchar(n)` length when available. Native YAML uses `source.column_transformations` with `kind: truncate|mask|hash`, `columns`, `length`, and hash `algorithm`, `salt`, and optional `version: v1|v2`. Transformation rules enter the source fingerprint; salts are represented there only by a SHA-256 digest. The PostgreSQL 17, MySQL 8.4, and SQL Server 2022 external gates cover initial snapshots, incremental snapshots, and streaming before/after values while preserving raw keys for progress and deduplication.
...
- Debezium-compatible column truncation, fixed masking, and V1/V2 hashing across initial snapshots, CDC before/after images, and incremental snapshots
...
The SQL Server 2022 Developer RTM-CU25 external gate verifies snapshot handoff, fetch-size-one continuation through update before/after pairs, mid-transaction checkpoint restart with preserved transaction ordinals, three capacity-one polling-session termination cycles with connection-identity replacement, every expected row, and first-seen source order, concurrent transactions ordered by commit LSN, retention fail-closed behavior, heartbeat/action-query, snapshot/CDC equality for the core type matrix plus XML, hierarchyid, geometry, and geography, four-part transformation selectors across snapshot and CDC values, three-part selectors across source-signaled incremental snapshots, raw signal-command isolation, in-process keyset restart, CDC-window concurrent-update deduplication, real-broker Kafka replay after a completed connector checkpoint but before the signal offset is committed, and cleanup. Geometry and geography retain the complete native SQL Server `Serialize()` payload as bytes; hierarchyid retains its canonical path and XML retains canonical text. See [examples/sqlserver.properties](examples/sqlserver.properties).

The database must have CDC enabled, SQL Server Agent must run the capture job, and the connector user needs source-table reads plus direct read access to the `cdc` schema. CI runs the separate SQL Server 2022 Docker portability gate on every push and pull request; its capacity-one output performs three session-kill/reconnect cycles by default, requires every subsequent CDC transaction, and asserts a four-part transformation selector on both snapshot and CDC values. The gate remains runnable locally with:

```bash
cargo test -p rustium-sqlserver --test sqlserver_docker -- --ignored --nocapture
```

When Docker runs on a remote host, select that Docker context and set `RUSTIUM_SQLSERVER_DOCKER_SSH_HOST`. The container port remains bound to the daemon host's loopback interface and the test creates a temporary authenticated SSH forward; `RUSTIUM_SQLSERVER_DOCKER_SSH_LOCAL_PORT` can pin the local port when required.
...
The Oracle connector uses the pure-Rust `oracle-rs` driver and Oracle LogMiner with the online catalog. It requires ARCHIVELOG mode, minimum supplemental logging, redo retention covering the checkpoint SCN, and the LogMiner catalog/query privileges documented by Debezium. Initial snapshots read selected tables with `AS OF SCN` at a fixed Flashback anchor; streaming reads committed rows from `V$LOGMNR_CONTENTS` and persists `scn`, `commit_scn`, transaction ID, and event serial in the checkpoint. Oracle DML generated by LogMiner is decoded conservatively: scalar literals are typed, supported date/timestamp/number/binary values remain typed, and expressions that cannot be proven lossless remain textual rather than being guessed.

Native YAML uses `source.type: oracle` with `hostname`, `port`, `username`, `password`, `database` (service name), optional `pdb_name`, `schemas`, `tables`, `log_mining_strategy: online_catalog`, `archive_log_only_mode`, `batch_size`, and `poll_interval`. Debezium properties map `OracleConnector`, `database.dbname`, `pdb.name`, `schema.include.list`, `schema.exclude.list`, `table.include.list`, `table.exclude.list`, `log.mining.strategy`, `log.mining.batch.size.default`, and `archive.log.only.mode`. `redo_log_catalog` is rejected until a redo dictionary implementation is available; it is never silently downgraded. The connector fails closed when ARCHIVELOG or supplemental logging is absent.

The connector crate is `rustium-oracle` (the crates.io publication is part of the next reviewed release). The external test is intentionally ignored until an Oracle instance with LogMiner privileges is supplied:

```bash
export RUSTIUM_ORACLE_TEST_HOST=oracle.example
export RUSTIUM_ORACLE_TEST_PORT=1521
export RUSTIUM_ORACLE_TEST_SERVICE=FREEPDB1
export RUSTIUM_ORACLE_TEST_USER=rustium
export RUSTIUM_ORACLE_TEST_PASSWORD='secret'
cargo test -p rustium-oracle --test oracle_external -- --ignored --nocapture
```

### MongoDB

The MongoDB connector uses the official Rust driver and Change Streams. MongoDB must run as a replica set or sharded cluster. Rustium opens the stream at an operation-time anchor before the initial snapshot, then persists the opaque Change Stream resume token on every delivered event. A restart uses `resume_after`; a checkpoint created at the snapshot boundary uses `start_at_operation_time`, so changes concurrent with the snapshot are not silently skipped.

Native YAML uses `source.type: mongodb` with `connection_string`, optional `databases`, `collections` (`include`/`exclude` regexes), `full_document: default|update_lookup`, `full_document_before_change: off|when_available|required`, `batch_size`, and `poll_interval`. Debezium properties map `MongoDbConnector`, `mongodb.connection.string`, `database.include.list`, `collection.include.list`, `collection.exclude.list`, `capture.mode` (`change_streams`, `change_streams_update_full`, or `change_streams_with_pre_image`), and `max.batch.size`. Inserts, replacements, updates, and deletes map to the common Rustium event model; `_id` is the document key, BSON documents/arrays/binary/ObjectId/Decimal128/date/timestamp values retain explicit typed representations, and unsupported change types are emitted as `message` events instead of being misclassified.

The connector crate is `rustium-mongodb` (the crates.io publication is part of the next reviewed release). The external test requires a replica-set MongoDB URI:

```bash
export RUSTIUM_MONGODB_TEST_URI='mongodb://127.0.0.1:27017/?replicaSet=rs0'
export RUSTIUM_MONGODB_TEST_DATABASE=cdc_demo
...
```text
 PostgreSQL WAL / MySQL binlog / SQL Server CDC / Oracle LogMiner / MongoDB Change Streams
              |
              v
          Source 连接器
              |
...
- 运行被忽略的 Oracle 外部测试时，需要启用 ARCHIVELOG、supplemental logging 和 LogMiner 权限的 Oracle
- 运行被忽略的 MongoDB 外部测试时，需要 MongoDB replica set 或 sharded cluster
...
仓库中持续编译校验的源码位于 [crates/rustium-cli/examples/embed_postgresql.rs](crates/rustium-cli/examples/embed_postgresql.rs)。将连接器配置保存为 `rustium.yaml` 后，可通过 `cargo run -p rustium-cdc --example embed_postgresql` 运行。发布到 crates.io 的 crate 名称是 `rustium-cdc`，安装后提供 `rustium` 二进制。
...
PostgreSQL、MySQL 和 SQL Server 列转换使用 Debezium 动态参数名。Selector 对 PostgreSQL 使用 `schema.table.column`，对 MySQL 使用 `database.table.column`，对 SQL Server 同时接受 `database.schema.table.column` 和 `schema.table.column`；它们都是 anchored、大小写不敏感正则，同一列按固定顺序 `truncate -> 固定 mask -> hash V1 -> hash V2` 选择第一个匹配规则。Java `.properties` 文件中，如需让正则保留用于匹配字面点号的转义，应写成 `\\.`。`column.truncate.to.<length>.chars` 截断字符或二进制值，`column.mask.with.<length>.chars` 即使遇到 SQL NULL 也输出指定数量的 `*`，两种 hash 都保留 NULL。Hash 接受常用 JCA digest 名称；V1 使用 Java `ObjectOutputStream` String serialization 兼容字节，V2 对 UTF-8 文本计算 hash。输出为小写十六进制，并在存在声明的 `char(n)`、`varchar(n)`、`nchar(n)` 或 `nvarchar(n)` 长度时截短。原生 YAML 使用 `source.column_transformations`。转换规则会进入 source fingerprint，但 fingerprint 只保存 salt 的 SHA-256 digest，不保存 salt 原文。PostgreSQL 17、MySQL 8.4 和 SQL Server 2022 外部门槛覆盖 initial snapshot、incremental snapshot 和 streaming before/after value，同时保证进度推进和去重仍使用原始 key。
...
- 在 initial snapshot、CDC before/after image 和 incremental snapshot 上执行兼容 Debezium 的列截断、固定 mask 与 V1/V2 hash
...
SQL Server 2022 Developer RTM-CU25 外部门槛已验证快照切换、fetch size 为 1 时跨 update before/after 的继续读取、保持事务序号的事务中间 checkpoint 重启、3 轮容量为 1 的 polling-session 终止循环及连接 identity 替换、全部预期记录和首次出现的源端顺序、按 commit LSN 排序的并发事务、retention fail-closed、heartbeat/action-query、核心类型以及 XML、hierarchyid、geometry、geography 的快照/CDC 一致性、四段 selector 在 snapshot 和 CDC value 上的转换、三段 selector 在 source signal 触发的 incremental snapshot 上的转换、signal command 原值隔离、in-process keyset 重启、CDC window 并发更新去重、connector checkpoint 已完成但 signal offset 尚未提交时的真实 broker Kafka 重放，以及资源清理。Geometry 和 geography 以字节保留完整的 SQL Server 原生 `Serialize()` payload；hierarchyid 保留规范路径，XML 保留规范文本。示例见 [examples/sqlserver.properties](examples/sqlserver.properties)。

数据库必须启用 CDC，SQL Server Agent 必须运行 capture job，连接器用户需要读取源表，并能直接读取 `cdc` schema。CI 会在每次 push 和 pull request 时运行独立的 SQL Server 2022 Docker 可移植性门槛；其容量为 1 的输出默认执行 3 轮 session 终止/重连，要求全部后续 CDC 事务，并在 snapshot 与 CDC value 上断言四段 transformation selector。本地仍可通过以下命令运行：

```bash
cargo test -p rustium-sqlserver --test sqlserver_docker -- --ignored --nocapture
```

Docker 位于远程主机时，先选择对应 Docker context，再设置 `RUSTIUM_SQLSERVER_DOCKER_SSH_HOST`。容器端口仍只绑定 daemon 主机 loopback，测试会建立临时认证 SSH forward；需要固定本地端口时可设置 `RUSTIUM_SQLSERVER_DOCKER_SSH_LOCAL_PORT`。
...
Oracle 连接器使用纯 Rust `oracle-rs` 驱动和 LogMiner 在线字典。数据库必须启用 ARCHIVELOG、minimum supplemental logging，并为连接账号授予 Debezium 文档所需的 LogMiner 目录/查询权限；redo 保留必须覆盖 checkpoint SCN。initial snapshot 在固定 SCN 上通过 `AS OF SCN` 读取选表，streaming 从 `V$LOGMNR_CONTENTS` 读取已提交记录。Checkpoint 持久化 `scn`、`commit_scn`、事务 ID 和事件序号；无法证明无损的 LogMiner expression 会保留文本，不会猜测字段值。

原生 YAML 使用 `source.type: oracle`，并配置 `hostname`、`port`、`username`、`password`、作为 service name 的 `database`、可选 `pdb_name`、`schemas`、`tables`、`log_mining_strategy: online_catalog`、`batch_size` 与 `poll_interval`。Debezium properties 支持 `OracleConnector`、`database.dbname`、`pdb.name`、schema/table filters、`log.mining.strategy`、`log.mining.batch.size.default` 和 `archive.log.only.mode`。redo dictionary 尚未实现，因此 `redo_log_catalog` 会明确拒绝。
