...
- PostgreSQL 14+ validation, Debezium-compatible publication autocreation, and managed or external slot ownership
...
- `database.sslmode`, `plugin.name`, `slot.name`, `publication.name`, `publication.autocreate.mode`, `replica.identity.autoset.values`, `publish.via.partition.root`
...
- PostgreSQL 14+ 校验、Debezium 兼容 publication 自动创建，以及托管或外部 slot 所有权校验
- 导出一致性快照与有界分页读取
