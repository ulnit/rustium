...
Checked-in golden fixtures pin the complete Debezium JSON destination, key, and envelope for PostgreSQL, MySQL, and SQL Server. CI compares generated records against those fixtures so connector-specific source metadata or topic-routing drift is reviewed explicitly.
...
仓库内置 golden fixture，固定 PostgreSQL、MySQL 和 SQL Server 的完整 Debezium JSON destination、key 和 envelope。CI 会将生成记录与这些 fixture 比较，使连接器特有 source 元数据或 topic routing 漂移必须经过明确审查。
