...
| PostgreSQL 14+ snapshot, `pgoutput`, persistent schema history, and heartbeat records | Implemented; destructive-DDL restart and heartbeat/action-query gates pass with PostgreSQL 17 |
...
| PostgreSQL 14+ 快照、`pgoutput`、持久 schema history 和 heartbeat record | 已实现；PostgreSQL 17 破坏性 DDL 重启及 heartbeat/action-query 门槛通过 |
...
测试会创建唯一命名的表、publication 和托管 replication slot，覆盖快照切换、事务顺序、checkpoint 停止、跨破坏性 DDL 的历史 `Relation` 重放、重启不重复快照、周期 heartbeat record、`heartbeat.action.query`、heartbeat 表过滤和资源清理。这些门槛已在启用 `wal_level=logical` 的 PostgreSQL 17 上通过。
