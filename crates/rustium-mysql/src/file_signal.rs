...
pub(crate) async fn read_and_clear(path: &str) -> Result<Vec<String>> {
    ...
    tokio::task::spawn_blocking(move || read_and_clear_blocking(Path::new(&path)))
        .await
        .map_err(|error| Error::Source(format!("MySQL signal file task failed: {error}")))?
}
...
fn read_and_clear_blocking(path: &Path) -> Result<Vec<String>> {
    let mut file = match OpenOptions::new().read(true).write(true).open(path) {
        ...
        Err(error) => {
            return Err(Error::Source(format!(
                "failed to read MySQL signal file {:?}: {error}",
                path.display()
            )));
        }
    };
    ...
    file.set_len(0).map_err(|error| {
        Error::Source(format!(
            "failed to clear MySQL signal file {:?}: {error}",
            path.display()
        ))
    })?;
    file.sync_data().map_err(|error| {
        Error::Source(format!(
            "failed to sync MySQL signal file {:?}: {error}",
            path.display()
        ))
    })?;
    Ok(contents
