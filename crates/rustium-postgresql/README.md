...
PostgreSQL `pgoutput` CDC source for Rustium. It supports exported-snapshot handoff, persistent schema history, WAL recovery, typed core and extension values, heartbeats, signals, and incremental snapshots.
...
Rustium 的 PostgreSQL `pgoutput` CDC source，支持 exported snapshot 切换、持久 schema history、WAL 恢复、核心及扩展类型值、heartbeat、信号和增量快照。
