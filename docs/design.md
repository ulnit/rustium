...
Every ordinary MySQL connection sets the session time zone to UTC so `TIMESTAMP` snapshot values match UTC binlog values. Binlog ENUM ordinals and SET bit masks are decoded through the captured column definition. The external type matrix requires snapshot/binlog equality for boolean, signed and unsigned integers, decimal, float/double, bit and binary, date/time/datetime/timestamp, string/text, JSON, ENUM, SET, and null values.
...
每个普通 MySQL 连接都会把 session time zone 设置为 UTC，使 `TIMESTAMP` 快照值与 UTC binlog 值一致。Binlog ENUM 序号和 SET 位掩码会根据已捕获列定义还原。外部类型矩阵要求 boolean、有符号/无符号整数、decimal、float/double、bit/binary、date/time/datetime/timestamp、string/text、JSON、ENUM、SET 和 null 在快照/binlog 路径上完全一致。
