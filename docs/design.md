...
The event loop schedules at most one incremental chunk per turn and only outside an active binlog transaction. Binlog records, pause/resume/stop controls, and external signal acknowledgements are therefore observed at chunk boundaries. Schema-history updates preserve the active incremental state instead of replacing it. A chunk commit atomically advances the keyset state with its delivered read records. The Kafka channel reuses the single-partition, checkpoint-coupled `rustium-signal-kafka` implementation and Debezium topic/key contract. MySQL does not yet implement Debezium's open/close binlog-window deduplication; concurrent changes remain visible as ordinary CDC events and can overlap an incremental read until that gate is implemented.
...
SQL Server supports Debezium-compatible source-table, file, bounded in-process, and Kafka signal channels. A source signal table may be named as `schema.table` or `database.schema.table`; it must contain exactly text-compatible `id`, `type`, and `data` columns in order and must have its own active CDC capture instance. Rustium includes that capture instance even when business table filters exclude it, validates its layout, consumes create operations as commands, and suppresses all signal-table rows from business output. External channels do not require a signal table. JMX configuration maps to the bounded in-process channel with an explicit compatibility warning.

`execute-snapshot` expands fully matched short or database-qualified collection expressions, requires a primary key, captures a fixed maximum key, and advances typed single/composite keysets. Connector state version 1 persists collection progress, additional conditions, current/maximum keys, chunk sequence, pause state, and a bounded history of completed or stopped signal IDs. One chunk runs per event-loop turn and only at a completed CDC commit boundary. Chunk rows and the advanced keyset are checkpointed together; source-table controls are checkpointed on their CDC commit, while Kafka offsets remain coupled to the runtime acknowledgement. `pause-snapshot`, `resume-snapshot`, and scoped `stop-snapshot` are idempotent across restart.

Like the current MySQL implementation, SQL Server does not yet buffer a chunk between open/close CDC watermarks. Concurrent CDC changes remain visible as ordinary events and can overlap an incremental read until CDC-window deduplication is implemented and externally tested.

Multiple database names require explicit partition-aware ordering and checkpoint ownership. Rustium rejects them until that contract is tested. The external gate has verified snapshot handoff, fetch-size-one update pairing, mid-transaction replay with preserved ordinals, concurrent commit ordering, retention fail-closed behavior, heartbeat/action-query, the core type matrix, in-process keyset restart, source-table signaling with additional conditions, and resource cleanup against SQL Server 2022 Developer RTM-CU25. Container portability, CDC-window deduplication, and extended spatial/special-type fixtures remain open.
...
2. Close MySQL binlog-window deduplication, Java TLS-store conversion, wider DDL/type, and real-broker Kafka recovery gates.
3. Close SQL Server CDC-window deduplication, container-portability, extended-type, and real-broker Kafka gates.
...
事件循环每轮最多安排一个增量 chunk，并且只在没有活动 binlog 事务时执行，因此 binlog record、pause/resume/stop 控制和外部信号确认都可以在 chunk 边界被处理。Schema history 更新会保留活动增量状态，不会覆盖它。Chunk commit 会将 keyset 状态与已投递 read record 原子推进。Kafka 复用单 partition、与 checkpoint 绑定的 `rustium-signal-kafka` 实现及 Debezium topic/key 合约。MySQL 尚未实现 Debezium 的 open/close binlog window 去重；在该门槛完成前，并发变化仍会作为普通 CDC event 可见，并可能与增量 read 重叠。
...
SQL Server 支持 Debezium 兼容的 source table、file、有界 in-process 和 Kafka signal channel。Source signal table 可以使用 `schema.table` 或 `database.schema.table`，必须按顺序且仅包含文本兼容的 `id`、`type`、`data` 列，并具有独立活动 CDC capture instance。即使业务 table filter 排除了它，Rustium 也会发现该 capture instance、校验布局、将 create operation 作为 command 消费，并从业务输出中抑制所有 signal-table 行。外部 channel 不要求信号表。JMX 配置会映射到有界 in-process channel，并发出明确兼容 warning。

`execute-snapshot` 会展开完整匹配的短名称或 database-qualified 集合表达式，要求主键、固定最大主键，并推进带类型的单列/复合 keyset。Connector state version 1 持久化集合进度、additional condition、当前/最大 key、chunk 序号、pause 状态，以及有界的已完成或已停止 signal ID 历史。事件循环每轮只在完整 CDC commit 边界执行一个 chunk。Chunk row 与推进后的 keyset 一起 checkpoint；source-table 控制在自身 CDC commit 上 checkpoint，Kafka offset 则继续与 runtime acknowledgement 绑定。`pause-snapshot`、`resume-snapshot` 和 scoped `stop-snapshot` 在重启后保持幂等。

与当前 MySQL 实现相同，SQL Server 尚未在 open/close CDC watermark 之间缓冲 chunk。在 CDC window 去重实现并通过外部测试前，并发 CDC 变化仍会作为普通事件可见，并可能与增量 read 重叠。

多个数据库名称需要显式的分区感知排序和 checkpoint 所有权。在该契约经过测试前，Rustium 会直接拒绝。外部门槛已在 SQL Server 2022 Developer RTM-CU25 上验证快照切换、fetch size 为 1 的 update 配对、保持序号的事务中间重放、并发 commit 排序、retention fail-closed、heartbeat/action-query、核心类型矩阵、in-process keyset 重启、带 additional condition 的 source-table signaling 和资源清理。容器可移植性、CDC window 去重和扩展空间/特殊类型 fixture 仍待补齐。
...
2. 补齐 MySQL binlog window 去重、TLS store、更广 DDL/类型和真实 broker Kafka 恢复门槛。
3. 补齐 SQL Server CDC window 去重、容器可移植性、扩展类型和真实 broker Kafka 门槛。
4. 只有完成前三项后才考虑其他数据库。
