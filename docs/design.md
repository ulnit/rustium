...
1. Close SQL Server container-portability and extended-type gates.
2. Add real-broker Kafka Sink delivery and failure-path gates.
3. Only then consider additional databases.
4. Add Schema Registry formats, packaging, security policy, operational runbooks, and stable upgrade migrations before `1.0`.
...
多个数据库名称需要显式的分区感知排序和 checkpoint 所有权。在该契约经过测试前，Rustium 会直接拒绝。外部门槛已在 SQL Server 2022 Developer RTM-CU25 上验证快照切换、fetch size 为 1 的 update 配对、保持事务序号的事务中间重放、并发 commit 排序、retention fail-closed、heartbeat/action-query、核心类型矩阵、in-process keyset 重启、带 additional condition 的 source-table signaling、CDC window 并发更新去重、connector checkpoint/Kafka offset 崩溃窗口中的真实 broker 重放和资源清理。容器可移植性和扩展空间/特殊类型 fixture 仍待补齐。
...
该门槛使用隔离的表/capture-instance 名称和 Kafka topic，等待 SQL Agent 初始化，并验证快照记录、fetch size 为 1 的事务继续读取、事务中间 checkpoint 重放、并发 commit 排序、retention 失败、heartbeat/action-query、核心快照/CDC 类型一致性、带 checkpoint 的 in-process keyset 重启、带 additional condition 的 source-table signaling、信号表隔离、已完成 signal ID 持久化、connector state 已持久化但 Kafka offset 尚未提交时的真实 broker 重放和资源清理。测试已在 SQL Server 2022 Developer RTM-CU25 上通过。
...
1. 补齐 SQL Server 容器可移植性和扩展类型门槛。
2. 补齐真实 broker Kafka Sink 投递与失败路径门槛。
3. 只有完成前两项后才考虑其他数据库。
4. 在 `1.0` 前补 Schema Registry 格式、打包、安全策略、运维手册和稳定升级迁移。
