...
Unlike the MySQL connector, SQL Server does not yet buffer a chunk between open/close CDC watermarks. Concurrent CDC changes remain visible as ordinary events and can overlap an incremental read until CDC-window deduplication is implemented and externally tested.
...
与 MySQL 连接器不同，SQL Server 尚未在 open/close CDC watermark 之间缓冲 chunk。在 CDC window 去重实现并通过外部测试前，并发 CDC 变化仍会作为普通事件可见，并可能与增量 read 重叠。
