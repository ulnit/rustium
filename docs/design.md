...
Deserialization compatibility does not invent missing history. A completed MySQL version 1 checkpoint has no safe schema baseline for older binlog events, so MySQL resume rejects it and requires a checkpoint reset plus one new initial snapshot. PostgreSQL and SQL Server can continue reading their version 1 checkpoints because their current recovery paths do not depend on connector state.
...
反序列化兼容不会凭空生成缺失历史。已完成的 MySQL version 1 checkpoint 没有可用于旧 binlog 事件的安全 schema 基线，因此 MySQL 会拒绝从中恢复，并要求重置 checkpoint 后执行一次新的 initial snapshot。PostgreSQL 和 SQL Server 当前恢复路径不依赖 connector state，因此仍可继续读取 version 1 checkpoint。
