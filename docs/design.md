...
Oracle and MongoDB are now implemented after the first three connectors passed their initial correctness and recovery work. Future connectors remain gated by the same correctness, recovery, type-coverage, and operational release contract.
...
- Adding lower-priority database connectors before the current five are complete.
...
前三个连接器通过首轮正确性与恢复工作后，Oracle 和 MongoDB 已完成实现。后续数据库仍必须通过相同的正确性、恢复、类型覆盖和运维发布门槛。
