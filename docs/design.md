...
Multiple database names require explicit partition-aware ordering and checkpoint ownership. Rustium rejects them until that contract is tested. The external gate has verified snapshot handoff, fetch-size-one update pairing, mid-transaction replay with preserved ordinals, concurrent commit ordering, retention fail-closed behavior, heartbeat/action-query, the core type matrix, and resource cleanup against SQL Server 2022 Developer RTM-CU25. Container portability, Debezium signaling/incremental snapshots, and extended spatial/special-type fixtures remain open.
...
This gate creates isolated table/capture-instance names, waits for SQL Agent initialization, and verifies snapshot rows, fetch-size-one transaction continuation, mid-transaction checkpoint replay, concurrent commit ordering, retention failure, heartbeat/action-query, core snapshot/CDC type equality, and cleanup. It has passed against SQL Server 2022 Developer RTM-CU25.
...
3. Close SQL Server signaling/incremental-snapshot, container-portability, extended-type, and Kafka gates.
...
多个数据库名称需要显式的分区感知排序和 checkpoint 所有权。在该契约经过测试前，Rustium 会直接拒绝。外部门槛已在 SQL Server 2022 Developer RTM-CU25 上验证快照切换、fetch size 为 1 的 update 配对、保持序号的事务中间重放、并发 commit 排序、retention fail-closed、heartbeat/action-query、核心类型矩阵和资源清理。容器可移植性、Debezium signaling/增量快照和扩展空间/特殊类型 fixture 仍待补齐。
...
该门槛使用隔离的表/capture-instance 名称，等待 SQL Agent 初始化，并验证快照记录、fetch size 为 1 的事务继续读取、事务中间 checkpoint 重放、并发 commit 排序、retention 失败、heartbeat/action-query、核心快照/CDC 类型一致性和资源清理。测试已在 SQL Server 2022 Developer RTM-CU25 上通过。
...
3. 补齐 SQL Server signaling/增量快照、容器可移植性、扩展类型和 Kafka 门槛。
4. 只有完成前三项后才考虑其他数据库。
