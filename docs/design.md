...
- live WAL-retention invalidation and live PostGIS/pgvector fixtures where installed;
...
1. Close PostgreSQL JMX signaling, transient-metadata, live WAL-retention invalidation, optional live PostGIS/pgvector, and real-broker Kafka recovery gates.
...
- live WAL retention invalidation，以及服务器已安装扩展时的 PostGIS/pgvector 实测；
- Kafka 端到端恢复测试。
...
1. 补齐 PostgreSQL JMX 信号、短暂元数据、真实 WAL retention invalidation、可选 PostGIS/pgvector 实测和真实 broker Kafka 恢复门槛。
